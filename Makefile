...
ifeq ($(HAVE_CONFIG_MK),)
# The Wayland protocol sources are generated by configure, so a tree
# configured before a protocol was added has no way to make the new
# object: ask for the reconfigure rather than fail on a missing file.
config.mk: configure qb/* gfx/common/wayland/generate_wayland_protos.sh
...
ifeq ($(HAVE_METAL), 1)
METAL_SHADER_SRCS := gfx/drivers/metal_shaders/Shaders.metal
...
clean:
	...
	$(Q)rm -f $(TARGET)
	$(Q)rm -f *.d
	$(Q)rm -f default.metallib gfx/drivers/metal_shaders/*.air
	$(Q)rm -rf $(BUNDLE)
